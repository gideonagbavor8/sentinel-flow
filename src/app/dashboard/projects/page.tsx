    ...
    <div className="space-y-8 p-8 min-h-screen bg-[var(--background)]">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-slate-400 mt-1">Manage your secure workspaces and folders.</p>
        </div>
        <NewProjectButton />
      </div>

      {projects.length === 0 ? (
        <div className="flex flex-col items-center justify-center p-16 bg-slate-800/30 border border-dashed border-slate-700/50 rounded-3xl mt-8">
          <div className="p-4 bg-slate-800/50 rounded-2xl mb-4">
            <FolderGit2 className="w-12 h-12 text-slate-500" />
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6 mt-8">
          {projects.map((project) => (
            <Link href={`/dashboard/projects/${project.id}`} key={project.id}>
              <div className="group flex flex-col h-full p-6 rounded-2xl bg-slate-800/40 border border-slate-700/50 hover:bg-slate-800/80 hover:border-indigo-500/50 hover:shadow-[0_0_30px_rgba(79,70,229,0.1)] transition-all duration-300">
                ...
                <div className="flex items-center justify-between text-xs font-medium text-slate-500 border-t border-slate-700/50 pt-4 mt-auto">
